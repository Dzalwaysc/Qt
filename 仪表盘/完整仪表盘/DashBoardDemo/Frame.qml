    ...
    Canvas{
        ...
        onPaint: {
            ...
            var drawLine = function(){
                ...
                ctx.beginPath();
                // 画一个梯形
                // 起始点:  (0, 仪表盘当前位置 + 仪表盘的高度/2 - 10)
                ctx.moveTo(0, 85);

                // 画 '-' : (仪表盘当前位置 - 45, 仪表盘当前位置 + 仪表盘的高度/2 - 10)
                ctx.lineTo(105, 85);

                // 画 '\' : (仪表盘当前位置, 仪表盘当前位置+仪表盘高度+20)
                ctx.lineTo(150, 190);

                // 画 '-' : (方向盘当前位置+方向盘宽度, 仪表盘当前位置+仪表盘高度+20)
                ctx.lineTo(510, 190);

                // 画 '/' : (方向盘当前位置 + 仪表盘的宽度 + 45 ,仪表盘当前位置 + 仪表盘的高度/2 - 10)
                ctx.lineTo(555, 85);
                ctx.stroke();
                ctx.restore();
            }
    ...
    Rectangle{
        ...
        MouseArea{
            ...
            onClicked: {
                ...
                frame.save("/Users/oliver/Desktop/DashBoardDemo/image/frame.png");
                console.log("hi");
            }
        }
    }
