...
Item {
    ...
    property int rainLength: 50  // 流星的长度
    property int rainAngle: 30    // 流星下落的角度
    property double speed: 10    // 流星下落的速度
...
//            }
            ...
            for (; nowCount<rainCount; nowCount++){
                var rain = new MeteorRain();
                rain.init();
                rain.draw();
                rains.push(rain);
        ...
        // 若直接在onPaint上重绘，如注释的那样运行，当速度变快时会有卡顿
        // 因此使用定时器
        Timer{
            id: timer
            repeat: true; running: false; interval: 10
            onTriggered: metorShower.requestPaint();
        }
    }
}
