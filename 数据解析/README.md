# 串口数据流接收
- 因为串口接收到的数据长度可能是不固定
- 还有在异步使用QSerialPort的时候，我不知道怎么接固定长度的内容
- 因此数据流接收就很重要。下面给出本文的介绍顺序

## 数传接收 <font color=red>radioRecvFlow</font>
有头有尾，报文长度不固定，报文中不含有头和尾。

## 北斗接收 <font color=red>gpsRecvFlow</font>
有头无尾，报文长度不固定，报文中不含有头。

- 这里因为水平有限，我只写出了对头进行截取的方法，因此只有当下一条信息也传进来了，我才能确定当前信息已经接收完毕。
- 即存在一条信息的延迟时间

## 遥控接收 <font color=red>handleRecvFlow</font>
有头无尾，报文长度固定，报文中含有头

- 比如遥控数据，头是*Rem*，中间的数据有*R027*，即报文中含有头。好在他的长度固定为29个字节。

## 代码区
```c++
QByteArray SerialPort::radioRecvFlow(QByteArray recvMsg)
{
...
```c++
QByteArray SerialPort::gpsRecvFlow(QByteArray recvMsg)
{
...
```c++
QByteArray SerialPort::handleRecvFlow(QByteArray recvMsg)
{
